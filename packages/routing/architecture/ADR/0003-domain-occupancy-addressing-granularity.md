...
The addressing model this record replaces governed three concerns through three separate, independently specialized mechanisms. None of them survives in the routing DESIGN as it now stands — this record retires all three, and the description that follows is the only remaining statement of what they were. Hierarchy was expressed by continuing the pathname, but only one domain per zone was permitted to do so: the *axial* domain. Occupancy fan-out across sibling domains was expressed by a dedicated query-string key per domain, capped at exactly one entry — a *parallel axis*. Per-occupant parameters were encoded two different ways depending on which of the first two mechanisms an occupant's domain used: matrix-params-shaped bare query keys for an axial or single-occupant parallel domain, and nested percent-encoded values for a compound-keyed one. That trichotomy had no room for projecting more than one occupant of one domain without inventing a fourth, bolted-on mode composing keys alongside the existing single-entry parallel axis — the real problem this record closes. Should hierarchy, occupancy fan-out, and per-occupant parameters continue to be governed by separate, independently specialized mechanisms — axial pathname continuation, a single-entry parallel-axis query key, and a bolted-on compound-key mode reserved for the multi-occupant case — or should all three be expressed as outputs of one uniform entry grammar?
...
* No privileged domain per zone — "at most one axial domain per zone", as the superseded model stated it, is a constraint with no principled basis beyond needing someone to own the pathname, and every mechanism built on top of it inherits that arbitrariness.
* Uniform resolution across depth and occupant count — resolution has always run per domain and independently at every level, reading only the entries carrying that level's own domain key (DESIGN §1.1), and it ran that way identically beneath an axial base and beneath a parallel-axis value; the addressing mechanism that produces those bases and values should not itself fork into three cases the resolution primitive underneath never needed.
...
* Bad, because a nested domain's own key restates its whole ancestor chain in every entry that domain contributes, spending URL length on repeated ancestry exactly as the rejected compound-key option does — across a domain's entries here rather than inside one key — and bounded only by the URL length budget this package's own PRD records (`PRD.md` §3.1, "URL length budget").
...
* `cpt-frontx-routing-adr-mount-trigger-ownership` — cross-referenced as a still-standing, orthogonal decision this record does not revisit: the shape of this package's own navigation-derived surface is unchanged, as is the ecosystem rule it serves; only how the URL back-projection helper's target entry is composed changes.
