...
- **Search** — the current URL's query string, without the leading `?` that introduces it. Stated explicitly because a host location object conventionally includes that character, so an adapter reading one must strip it rather than pass it through: a query string still carrying it parses as a single unrecognized segment and loses every entry it holds. The grammar parser strips and reports a leading delimiter that reaches it anyway (§3, Grammar Parse, step 1.1), so the mistake costs a report rather than a blank screen — but this, here, is the shape an adapter is required to report.
- **Hash** — the current URL's fragment, without the leading `#` that introduces it, on the identical terms as Search above; the empty string when the current URL carries no fragment at all.
...
**Rationale**: A realm-global key, not a module-scoped variable, is what makes the instance reachable across independently bundled copies of this package — each copy is its own module graph and cannot see another copy's module-scoped state, but every copy runs in the same realm and can see the same global. The key carries the `NavigationHistory` contract's own version rather than naming the package alone, so a copy built against an incompatible future or past contract version resolves under its own key and constructs its own instance instead of silently capturing and reusing one whose shape it cannot actually satisfy — trading a same-version sharing guarantee for a per-version instance instead of a single instance quietly used the wrong way (PRD §12 records this as a bounded risk: prevention holds within one contract version, and a cross-version mismatch yields one instance per version, without a detection mechanism of its own). Registering the underlying browser subscription at construction, rather than waiting for a first `subscribe` call, is what keeps `location` from being stale for a caller who only ever reads it (§2 permits reading without subscribing) — a lazily-registered subscription would leave `location` frozen at its construction-time value for exactly the callers who never subscribe. A version-carrying string key, held for the page's own lifetime with no retain/release discipline of its own, is deliberate rather than an omission. Retain/release is a live convention among this ecosystem's neighbouring realm-globals: the shared fetch cache keeps a retainer count beside itself under its own well-known key (`packages/api/src/sharedFetchCache.ts`), and the lazy-loader registry mints and drops one resolver per load under `globalThis.__FRONTX_LAZY__` (`packages/mfes/src/lazy-loader/lazy-loader-registry.ts`). So the question is not whether a realm-global can carry a count — two of them next door do — but what a count would decide here. A retainer count names the moment a shared thing may be torn down, and both neighbours have such a moment: the fetch cache holds cached responses and in-flight requests that must be discarded once the last holder lets go, and the lazy-loader registry holds a per-load resolver that must go when its load does, or a sibling load resolves against a chain that no longer exists. This singleton has no such moment, because it owns nothing it could release. The navigation history it wraps is the browser's own — page-lifetime by nature, and not this instance's to dispose — and the only state that accumulates per holder, the subscriber registry, is already released one entry at a time through the unsubscribe function `subscribe` returns. Dropping the instance at a count of zero would be incorrect rather than merely useless: a later caller in the same realm must be handed the identical instance an earlier holder had, not a second one built over the same underlying history. A count here would compute a number no step is permitted to act on. What the key carries instead is the contract's own version, because a well-known key without it would not tell two incompatible copies apart in the first place, which is the entire reason the key carries a version at all.
...
4. [x] - `p1` - **Dispatch a round** (`inst-dispatch-round`, invoked by both step 2 and step 3) - `inst-dispatch-round`
   ...
   2. [x] - `p1` - **FOR EACH** callback in that snapshot, in registration order - `inst-foreach-subscriber`
      1. [x] - `p1` - **IF** that callback is still present in the live subscriber registry at the moment this iteration reaches its slot — i.e., nothing has unsubscribed it since the snapshot was taken - `inst-if-still-live`
         ...
         2. [x] - `p1` - **CATCH** an error thrown by the callback - `inst-catch-subscriber-error`
            ...
            2. [x] - `p1` - Report that error through this package's own diagnostic channel before continuing with the round — isolation is what the remaining callbacks are owed, but a callback that throws on every navigation must not thereby become indistinguishable from one doing its job, which is what an unreported error leaves it as: no failure propagates and nothing else records it, so the only visible symptom is whatever that callback was meant to update never updating - `inst-report-subscriber-error`
   ...
   4. [x] - `p1` - **IF** a callback triggers a new navigation during this round (reentrant navigation) - `inst-if-reentrant-navigation`
      ...
      2. [x] - `p1` - Deferral is bounded: once a fixed number of deferred rounds have run consecutively without the deferral queue ever emptying, abandon whatever is still queued and **THROW** a clear error naming that bound, rather than serving the queue further. A callback that defers a fresh round from every round it receives is a feedback loop, not a cascade — and because deferral is a queue drained in a loop rather than a nested call, an unbounded one exhausts no stack, raises nothing, and simply stops the realm from making progress for as long as it stays open. The bound sits far above any cascade this feature describes (a guard that redirects once, a canonicalizer that rewrites once, one round per subscriber in a realm whose subscriber count is itself small) and far below any duration a person would experience as a hang. The queue is abandoned before the throw, so the next genuine navigation starts from an empty one instead of inheriting a backlog the breach already proved untrustworthy - `inst-reentrant-round-limit`
...
1. [x] - `p1` - Split the input into the shell subroute (everything before the first `?`), the query string (between `?` and `#`, or the rest of the string if no `#` is present), and the hash (everything from `#` onward, or absent) - `inst-split-url`
   1. [x] - `p1` - **IF** the caller supplied the three parts already split, and its query string still begins with `?`, or its hash still begins with `#`: strip that leading delimiter and report it through this package's own diagnostic channel, then continue parsing the stripped value. This branch receives the halves splitting produces, not the delimiters that introduce them; a host location object carries each delimiter by convention, so an adapter passing either value straight through hands one over. Stripping rather than rejecting, because this algorithm never throws and a convention mismatch in one adapter must not blank out a deep link a person actually opened — every entry in that query string would otherwise be lost whole, since the delimiter rides into the first raw entry string and fails its candidate domain key, leaving no entry, no warning, and nothing at all to see. Reporting rather than stripping quietly, because the adapter is genuinely wrong and its author is the only party able to correct it - `inst-normalize-split-delimiters`
...
5. [x] - `p1` - **FOR EACH** raw entry string, in order - `inst-foreach-raw-entry`
   ...
   6. [x] - `p1` - **FOR EACH** param segment, in order - `inst-foreach-param-segment`
      ...
      5. [x] - `p1` - **IF** the decoded `name` is the empty string — a segment carrying no name at all: an empty segment between two `;` (`sheet=a;;b=1`), a trailing `;` (`sheet=a;`), or a segment that is only a value (`sheet=a;=x`) - `inst-if-empty-param-name`
         1. [x] - `p1` - Drop the *whole entry* this segment belongs to on exactly the terms `inst-drop-entry-malformed-escape` above already sets — record a warning citing the entry's raw text and "malformed entry", *also* append it to the output foreign-segments list, and continue to the next raw entry string, abandoning whatever params of this entry were already collected — because `param-name = 1*( pchar-safe | pct-encoded )` (ADR 0003, "Tokens") requires at least one character, unlike `param-value`, whose otherwise identical production admits the empty string. A named param with no value (`;k`, `;k=`) is therefore well-formed and keeps the entry, while a value with no name does not: the two sides of `=` are not symmetric here - `inst-drop-entry-empty-param-name`
      6. [x] - `p1` - **IF** a param of this same `name` was already collected earlier in this same entry - `inst-if-duplicate-param`
      ...
      7. [x] - `p1` - **ELSE** append `{name, value}` to this entry's own ordered `params` list - `inst-append-param`
...
- [x] A listener that throws during dispatch does not prevent delivery to the remaining listeners in the same fan-out round, and that error is reported rather than discarded, so a listener failing on every navigation is distinguishable from one succeeding.
- [x] A listener that triggers a new navigation from every round it receives does not run forever: after a fixed number of consecutive deferred rounds the queue is abandoned and an error naming that bound is thrown, instead of the realm losing the ability to make progress with nothing reported at all.
- [x] A query string handed to the parser already split but still carrying its leading `?` — the shape a host location object reports, passed through verbatim by an adapter — parses to the entries it holds rather than to zero entries, and the mismatch is reported; the same holds for a fragment still carrying its leading `#`.
...
- [x] A param segment carrying no name at all — `sheet=a;;b=1`, a trailing `sheet=a;`, or the name-less `sheet=a;=x` — drops the whole entry it belongs to with a `malformed-entry` warning and keeps the raw text as a foreign segment, exactly as a malformed escape does, while a param carrying a name and no value keeps the entry: `sheet=a;k` and `sheet=a;k=` both parse to the single param `{name: 'k', value: ''}`, which is what lets a bare name and an explicitly empty value mean the identical thing on a round trip.
