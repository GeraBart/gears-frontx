...
- **Foreign segments** (H3, review round 20) — a query segment this package never treats as its own business: no `=` between a candidate domain key and extension, or a candidate domain key failing the `domain-key` production. Such a segment produces no warning, because it is not a malformed attempt at an entry, only a value some other party put in the same query string (an OAuth callback's `state`, an analytics `utm_*` parameter) — kept verbatim rather than the earlier, incidental behaviour of surviving only when it happened to also satisfy the entry grammar by coincidence. A segment that *does* satisfy the `domain-key` production but still fails to become an entry — an invalid extension token, or a param breaking the entry rules further in — still warns `malformed-entry` exactly as before, and is *also* collected as a foreign segment, on the same terms as a plainly-foreign one; this is the line between "not this package's business" (silent) and "looks like an entry but is broken" (warned) — both are equally foreign for the purpose of not being silently discarded. A segment dropped only for colliding with an earlier one under the same domain key (`duplicate-extension`) is neither: it *did* parse as an entry, so it is not collected here.
...
5. [x] - `p1` - **FOR EACH** raw entry string, in order - `inst-foreach-raw-entry`
   ...
   2. [x] - `p1` - **IF** the head segment carries no `=`, or the text before that `=` (the candidate domain key) does not conform to the `domain-key` production — an odd count of `.`-separated segments, each segment matching the `name` alphabet (H3, review round 20) - `inst-if-plainly-foreign`
   ...
   3. [x] - `p1` - **IF** the text after the head's `=` (the candidate extension) does not match the `name` alphabet - `inst-if-malformed-head`
      1. [x] - `p1` - This raw entry string has a valid-looking domain key but is still not a well-formed entry: drop it, record a warning citing its raw text and "malformed entry", *also* append it to the output foreign-segments list (on the same terms as step 5.2.1 above — this package still keeps the text verbatim, it only additionally warns because this one looked close enough to a real entry to be worth flagging), and continue to the next raw entry string - `inst-drop-malformed`
...
**Rationale**: Malformed-entry, duplicate-extension, and duplicate-parameter are three independent edge rules the grammar deliberately resolves differently — a malformed entry is discarded outright because there is no well-formed fact to keep as an *entry* (though its raw text still survives as a foreign segment, below); a duplicate extension under one domain key keeps the first occurrence because entries are an ordered list a domain reads meaning from; a duplicate parameter name keeps the last occurrence because parameters are a map, the same rule `URLSearchParams` itself would apply if this grammar's own delimiters let it be used at all. Every rule reports a warning rather than throwing, because a consumer resolving an existing, possibly bookmarked URL cannot recover from a thrown parse error the way it can from a dropped or overwritten entry. Foreign segments (H3, review round 20) exist because a query string is shared ground: a segment this package cannot make an entry out of is not, by that fact alone, wrong — it may simply belong to another party (an OAuth callback's `state`, an analytics `utm_*` parameter) — so step 5.2 keeps it verbatim rather than the earlier behaviour of losing it, or keeping it only by the accident of its also happening to satisfy the entry grammar. Step 5.3's warning stays because a segment whose domain key *does* look intentional but whose extension does not is more likely a real mistake worth surfacing than an unrelated party's own parameter.

### Grammar Serialize

- [x] `p2` - **ID**: `cpt-frontx-algo-routing-navigation-substrate-grammar-serialize`

**Input**: A shell subroute, an ordered entry list of `{domainKey, extension, params}` (`params` itself ordered), an ordered foreign-segments list, and a hash — the identical shape `cpt-frontx-algo-routing-navigation-substrate-grammar-parse` produces.

**Output**: A URL string, or a thrown error naming the offending entry (or, for step 0, the offending shell subroute) when the input violates a structural invariant this package itself enforces on write.

**Steps**:
0. [x] - `p1` - **IF** the given shell subroute contains `?`, `#`, or `&` (H1, review round 20) - `inst-if-invalid-shell-subroute`
...
- [x] Parsing a query string in which every domain is at zero occupants — an empty query string — and serializing the resulting empty entry list back reproduces the bare shell subroute alone, with no trailing `?`:
  ...
  /en
  ```
- [x] A raw entry that is plainly not an entry at all — no `=` between a candidate domain key and extension, or a candidate domain key with a token outside the `name` alphabet or an even segment count, e.g. `/en?screen&a.b=x&widgets=line-a;range=7d` — is kept as a foreign segment, verbatim, with no warning (H3, review round 20): here, `screen` has no `=` at all, and `a.b=x` is its own segment whose candidate domain key `a.b` has an even segment count (two), so both are foreign, while `widgets=line-a;range=7d` is kept as an entry and parsing does not throw.
- [x] A raw entry whose candidate domain key does conform, but whose extension does not match the `name` alphabet, e.g. `screen=Dashboard`, is dropped from the parsed entry list with a warning citing its own raw text and "malformed entry" — it looks enough like an entry to warn about, unlike the plainly-foreign case above — and is also kept as a foreign segment, on the same terms as one; a raw entry dropped only for colliding with an earlier one under the same domain key (`duplicate-extension`) is neither warned as malformed nor kept foreign, because it did parse as a well-formed entry.
