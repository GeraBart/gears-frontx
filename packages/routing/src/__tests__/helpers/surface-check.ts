...
// `dist/index.d.ts` — pulled out here so they can
...
// a mutation of the checked-in build output
...
// Must recognise not only `declare type`/`declare interface`/a bare
// `type X =`, but also a bare `interface X {` with no `declare` keyword,
...
// RouteSignal {`) — recognising only the `declare`-prefixed forms would
// make the "declared" half of this check dead
// code for every interface pinned today, since the assertion would only
// ever pass via the `exported` fallback below. Accepts `interface`/
