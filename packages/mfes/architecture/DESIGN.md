...
A chain's execution is origin-invariant: the branch sequence it selects and the settlement class each of its actions reaches depend only on the chain as declared, on the reachable topology and handler behaviour at execution time, and on durations chargeable to the chain itself — never on which runtime, nesting depth, or lifecycle stage dispatched it. Dispatching the same chain from the shell, from a mid-tree registry, or from a leaf composes to the same handler-and-fallback sequence and the same per-node settlement, because resolution, timeout accounting, and branch selection are executor-internal decisions the origin cannot see or influence, only trigger.

The guarantee is conditional, not absolute, and is stated here as conditional: it holds for origins within one connected routing tree, once registration propagation between those origins has converged, absent concurrent mutation of that topology while the chain is in flight, and subject to a fourth qualification on transport liveness across hops that compounds with nesting depth and is carried forward below. The qualification is real, not defensive. A registry constructed outside the synchronous mount window that would have let it adopt an inbound bridge behaves, correctly, as a root holding no such bridge — there is no sound way to attribute a late-constructed registry to the extension whose mount produced it — so a target reachable from an ancestor of that extension is genuinely unreachable from the detached registry, not merely slower to reach. Claiming origin invariance without this qualification would assert a reachability guarantee the topology itself does not provide.
...
Emitter-facing dispatch is acceptance-only: a call that hands a chain to the runtime accepts it or refuses it, and reports nothing about what the chain then did. A refusal is raised synchronously at the call itself, never as a value or a rejected promise the emitter could await, so no emitter-facing dispatch point yields anything an emitter could await for chain execution — origin invariance and an awaitable emitter surface are mutually exclusive, because an awaiting emitter is itself a second, origin-specific path by which a chain's progress could be observed and acted on. Settlement observation stays internal to the executor that dispatched the chain, which is what lets the chain select its own `next` or `fallback` continuation without an emitter racing or duplicating that decision.

Only two kinds of duration count against a chain's declared budget: time a handler spends settling, and time the executor itself spends on continuation work between nodes. Time spent carrying a chain across a bridge hop is not chargeable, so a chain dispatched from deeper in the tree costs more wall-clock time to complete but draws down no more of its declared budget than the same chain dispatched at the root. Continuing the fourth qualification named above, transport liveness across a hop is governed separately, by one bound fixed identically at every hop rather than a caller-supplied or per-origin value, and a transport failure under that bound is an environmental failure mode explicitly outside the scope of this constraint's guarantee: a chain crossing N hops faces N independent chances to hit that bound, and no single per-hop value, however small, makes the aggregate exposure origin-invariant.

Three things stay outside what this constraint settles. It does not unify how a retraction of routing state resolves for a chain executing locally against how it resolves for one executing across a bridge hop: retracting a forwarding entry force-rejects the forwarded action a hop already holds in flight, draining that hop's per-target in-flight tracking for the retracted target, while retracting a target locally instead refuses the unregistration itself for as long as actions for that target remain pending — force-rejection of the in-flight action on one side against refusal of the retraction on the other, two structurally different resolutions, which is exactly why concurrent topology mutation is excluded above rather than folded into the guarantee. It does not extend origin invariance to reachability between disconnected routing trees or between trees whose registration propagation has not yet converged; those are outside the connected-tree qualification, not a gap in it. And it does not specify a correlation protocol for a terminal completion signal — the mechanism an emitter that genuinely needs to know a chain finished is expected to declare as a node inside the chain itself rather than by awaiting the dispatch call; that protocol's shape is separate work this constraint leaves unsettled.
...
sequenceDiagram
    ...
    alt a recorded forwarding entry resolves at every hop
        ...
        Shell->>Shell: settle via the chain's own declared fallback
    end
