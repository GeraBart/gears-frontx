...
ALWAYS route to the `pr-review` workflow WHEN user intent matches PR review patterns:
...
ALWAYS route to the `pr-status` workflow WHEN user intent matches PR status patterns:
...
ALWAYS route to the `migrate-openspec` workflow WHEN user intent matches OpenSpec migration patterns:
