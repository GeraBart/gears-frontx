...
_LEGACY_CPT_MARKER = "@cpt:root-agents"
        ...
        if os.path.isfile(agents):
            try:
                with open(agents, encoding="utf-8") as f:
                    head = f.read(512)
                if _CF_MARKER in head or _LEGACY_CPT_MARKER in head:
                    return current
            except OSError:
                pass
...
def _read_cf_studio_path(project_root):
    """Read cf-studio-path variable from AGENTS.md TOML block.

    Parses the ```toml block inside the @cf:root-agents section
    (or the legacy @cpt:root-agents section for un-migrated projects).
    Returns the value (e.g. '.cf-studio', '.cypilot', 'cypilot', 'cpt') or None.
    """
    ...
    try:
        ...
        return None
    if _CF_MARKER not in content and _LEGACY_CPT_MARKER not in content:
        return None
    # Extract cf-studio-path (or legacy cypilot_path) from TOML block
    m = re.search(
...
_CF_STUDIO_REL = _read_cf_studio_path(ROOT)
if _CF_STUDIO_REL is None:
    ...
    for _candidate in (".cf-studio", ".cypilot", "cypilot", "cpt"):
        if os.path.isdir(os.path.join(ROOT, _candidate)):
            _CF_STUDIO_REL = _candidate
            break
    if _CF_STUDIO_REL is None:
        _CF_STUDIO_REL = ".cf-studio"

CF_STUDIO_PATH = os.path.join(ROOT, _CF_STUDIO_REL)
...
def _load_pr_config():
    ...
    # TOML candidates
    candidates_toml = [
        os.path.join(CF_STUDIO_PATH, "config", "pr-review.toml"),
        os.path.join(CF_STUDIO_PATH, ".gen", "kits", "sdlc", "scripts", "pr-review.toml"),
    ]
    ...
    candidates_json = [
        os.path.join(CF_STUDIO_PATH, "config", "pr-review.json"),
