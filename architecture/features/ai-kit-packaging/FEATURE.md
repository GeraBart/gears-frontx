...
2. [x] - `p1` - Cypilot CLI locates the kit registration at `[kits.cyber-pilot-kit-frontx]` in `core.toml` - `inst-locate-registration`
...
4. [x] - `p1` - Read the kit's `manifest.toml` from the path recorded in `[kits.cyber-pilot-kit-frontx].path` - `inst-read-manifest`
...
7. [x] - `p1` - **FOR EACH** resource entry in the validated manifest `resources` array - `inst-for-each-resource`
   1. [x] - `p1` - Resolve the resource's installed path from `[kits.cyber-pilot-kit-frontx.resources.<id>].path` - `inst-resolve-resource-path`
...
**Steps**:
1. [x] - `p1` - Confirm the manifest object has the required top-level fields: `manifest` and `resources` - `inst-check-required-fields`
2. [x] - `p1` - Confirm `manifest.version` is present and non-empty - `inst-check-version`
3. [x] - `p1` - Confirm `resources` is a non-empty array - `inst-check-resources-array`
4. [x] - `p1` - **FOR EACH** entry in `resources` - `inst-for-each-entry`
   1. [x] - `p1` - Confirm the entry has required fields: `id`, `source`, `default_path`, `type` - `inst-check-entry-required`
...
**Transitions**:
1. [x] - `p1` - **FROM** `PACKAGED` **TO** `INSTALLED` **WHEN** the Cypilot CLI installs `cyber-pilot-kit-frontx` into the consuming project and records the kit registration at `[kits.cyber-pilot-kit-frontx]` in `core.toml` with its `path`, `version`, and `source` - `inst-transition-packaged-to-installed`
...
4. [x] - `p1` - **FROM** `INSTALLED` **TO** `PACKAGED` **WHEN** the Cypilot CLI uninstalls or removes the kit from the consuming project, deleting the `[kits.cyber-pilot-kit-frontx]` registration and installed resources - `inst-transition-installed-to-packaged`
...
- [x] `cyber-pilot-kit-frontx` installs through the Cypilot CLI and the kit registration appears at `[kits.cyber-pilot-kit-frontx]` in `core.toml` with `path`, `version`, and `source` fields
- [x] Every resource `id` declared in the kit's `manifest.toml` begins with the literal prefix `frontx_` (KIT-1); a CI check on the manifest asserts all ids match `^frontx_`
- [x] The kit manifest validates against the kit-manifest schema (`manifest` + non-empty `resources[]`; required fields present on every entry)
