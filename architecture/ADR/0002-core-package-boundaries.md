...
The routing capability falls inside this partition and is decided here on the same reasoning: it is delivered as an engine-agnostic navigation substrate — a core member carrying no dependency on any concrete router engine — together with a separate engine-provider package implementing the port that substrate declares. This split is binding, and it obliges three things: the navigation substrate depends on no concrete router engine and on no other package in this ecosystem, its own engine provider included, so the edge between the two runs one way only, from provider to substrate; an engine-provider package is the only package in this ecosystem permitted to import a concrete router engine; and that dependency on the substrate is an engine-provider package's sole intra-ecosystem edge. Being bound to a concrete engine and a concrete UI framework, an engine-provider package is a full published-libraries member that is not core, so it sits outside this partition's boundary constraints rather than extending them. The rules that carry these obligations are owned by the two members themselves, not by this record: ROUTING-1 and ROUTING-2 in [packages/routing/architecture/DESIGN.md](../../packages/routing/architecture/DESIGN.md) §2.2, and ROUTING-TANSTACK-1 in [packages/routing-tanstack/architecture/DESIGN.md](../../packages/routing-tanstack/architecture/DESIGN.md) §2.2.
...
* Good, because the routing capability's concrete engine dependency is confined to a separately published member, so the navigation substrate stays inside the core subset and a consumer can replace the engine provider without touching the substrate or any other member.
...
* Bad, because a consumer that needs routing takes on two coordinated dependencies, released on their own lines, rather than one.
...
The routing split is confirmed against the rules its two members' own DESIGNs define rather than against this record's core constraints: review, together with the import-graph guard those rules are written for, confirms that the navigation substrate imports no concrete router engine and no other package in this ecosystem, and that the engine provider is the only package importing a concrete router engine, reaching the substrate as its sole intra-ecosystem edge.
