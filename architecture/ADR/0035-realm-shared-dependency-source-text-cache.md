...
* Good, because memory is bounded once for the realm rather than once per handler, so the bound no longer multiplies with a composition's handler count.
