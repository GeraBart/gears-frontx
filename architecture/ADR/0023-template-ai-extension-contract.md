...
* **Right altitude** — the decision must fix the conformance shape without fixing a complete field-level schema, which belongs to the owning FEATURE (`cpt-frontx-feature-template-ai-extensions`) per `cpt-frontx-adr-contract-schema-ownership`, not to a decision record and not to DESIGN.
...
This decision is at decision altitude: it fixes the existence of a declared contract and the categories it admits. It does NOT define the complete field-level schema of the declaration — that is owned by the FEATURE `cpt-frontx-feature-template-ai-extensions` per `cpt-frontx-adr-contract-schema-ownership`, not by this decision record or DESIGN (DATA-ADR-NO-001). The scope is the conformance shape of a template's AI-extension bundle; it does not decide how the bundle is found or composed once a template is installed (that is `cpt-frontx-adr-extension-discovery-activation`), nor how the framework itself is packaged (that is `cpt-frontx-adr-ai-tooling-framework-packaging`). The convention-only option offers no point at which a malformed declaration can be reported and lets misplacement pass silently; the inline-in-metadata option overloads the template's existing metadata and couples AI extensions to fields that serve unrelated purposes, weakening both recognizability and validatability.
...
* Neutral, because the concrete field-level schema is owned by the FEATURE `cpt-frontx-feature-template-ai-extensions`, so this decision constrains structure without specifying every field.
...
* Neutral, because it fixes categories at decision altitude and delegates the concrete schema to the owning FEATURE `cpt-frontx-feature-template-ai-extensions`.
...
Applicability of the remaining checklist categories: **PERF** — Not applicable, because no latency or throughput budget is bound to the bundle's conformance shape. **SEC** — Not applicable, because the contract introduces no secret material and no authentication surface. **REL** — Not applicable, because there is no service-availability target attached to a carried declaration. **DATA** — addressed by exclusion: this decision deliberately does NOT define the full field-level schema (DATA-ADR-NO-001); the schema is owned by `cpt-frontx-feature-template-ai-extensions` per `cpt-frontx-adr-contract-schema-ownership`. **OPS** — Not applicable, because there are no operational procedures for a carried bundle. **COMPL** — Not applicable, because no regulatory obligation bears on the bundle shape. **ARCH-ADR-008 (supersession)** — Not applicable, because this ADR supersedes no live ADR. **Review cadence**: revisit if a required extension kind does not fit the four-category model, or if the field-level schema owned by `cpt-frontx-feature-template-ai-extensions` forces a category boundary to change.
