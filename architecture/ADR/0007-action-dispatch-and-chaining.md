...
* Reachability across nesting depth without probing — a target nested arbitrarily many runtime boundaries away must be reachable in a single addressed hop-by-hop path, not through fan-out to every descendant; this reinforces, for a reason not weighed at the time, the existing exclusion of the publish/subscribe alternative — so depth-spanning delivery must be achieved by carrying routing state along the mediator chain rather than by broadcasting.
...
* Bad, because propagated advertisements and forwarding entries are additional per-registry state that must be kept consistent with admission and disposal, including retraction of every propagated advertisement and rejection of in-flight actions for a disposed subtree.
