...
* Good, because declared ownership boundaries give the conflict check an authoritative source to compare, and declared references give preset resolution an authoritative set to apply.
...
* Good, because declared boundaries feed the conflict check and declared references feed preset resolution.
...
* `cpt-frontx-contract-template-manifest` — This decision fixes the role, declared categories (identity, version, ownership boundaries, referenced templates), and produce-once / consume-many lifecycle of the manifest contract every template conforms to.
