...
* Completeness for an isolated load — the description must carry everything an isolated load needs: the base location for assets, the file backing each exposed entry, the entry's stylesheet assets, and the full set of shared-dependency declarations in a stable enumeration order — so a load needs no out-of-band lookup. That enumeration order fixes the sequence in which a consumer visits the declarations; it is not a load order. Resolving the declarations leaves-first remains the consumer's own pass over the sources it fetches, which is where dependency-order correctness and circular-dependency handling are decided (`cpt-frontx-adr-mfe-load-isolation`). Every mention of ordered declarations elsewhere in this decision carries this meaning.
