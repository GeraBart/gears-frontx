...
and the left menu fills in with screens contributed by the microfrontends. Each
MFE also serves its own preview build: demo `:3001`, blank `:3099`, widget
fixtures `:3201` / `:3202`.
...
MFEs are the unit of composition. Start from the blank MFE:
...
`src-app/mfe_packages/` — there is no registry file to edit.
