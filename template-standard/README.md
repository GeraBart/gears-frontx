...
```bash
frontx seed frontx-template-standard ./my-app
```
...
- **Demo microfrontends** (`src-app/mfe_packages`) — `demo-mfe` (Hello World,
  Profile, Current Theme, UIKit Elements, Widgets Host), a minimal `_blank-mfe`
  to copy from, and two widget fixtures.
...
# 3. Start everything — host + all microfrontend dev servers
npm run dev:all

# 4. Open the app
#    Host:  http://localhost:5173
#    MFEs:  :3001 (demo) · :3099 (blank) · :3201/:3202 (widget fixtures)
```
...
│   └── mfe_packages/        # Microfrontends (demo-mfe, _blank-mfe, fixtures)
