# Guideline: GTS ID Conventions (frontx-template-standard)

This template's MFE packages (`src-app/mfe_packages/*/mfe.json`) identify every
...
namespace and naming pattern **this template** actually uses across its own MFE
packages (`demo-mfe`, `_blank-mfe`, `widgets-fixture-a`, `widgets-fixture-b`), so
new MFEs added to a project scaffolded from this template stay consistent with the
existing ones.
...
- `~<namespace-path>.v1` — the solution-specific instance segment this template's
...
## Observed ID families in this template

| Family | Fixed prefix | This template's instance pattern | Real example |
|---|---|---|---|
...
`{app}` in every entry above is this template's own namespace root (`demo`,
`widgets`, `blank`) — a Project Developer forking this template for a real
...
by every MFE package in this template:
...
## Rule for new MFE packages in this template
