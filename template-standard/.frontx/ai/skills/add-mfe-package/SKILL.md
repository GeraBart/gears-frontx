---
name: frontx-template-standard-add-mfe-package
description: "Scaffold a new microfrontend (MFE) package inside this template (frontx-template-standard) — copy the _blank-mfe reference scaffold, assign a port and GTS identifiers that conform to this template's naming scheme, and register it with the host's manifest pipeline."
---

# Add an MFE Package (frontx-template-standard)

This skill is specific to **this template**. It relies on this template's concrete
...
The Project Developer wants to add a new microfrontend screen to a project scaffolded
from this template (a new screenset entry, a new isolated UI unit to mount into an
extension domain).
...
2. **Pick a free port** — this template's convention reserves `3001` for `demo-mfe`;
...
4. **Assign GTS IDs** — rewrite every placeholder ID in `mfe.json` following this
   template's ID taxonomy (see the `gts-id-conventions` guideline and the
...
7. **Verify** — `npm run type-check`, `npm run arch:check` (dependency-cruiser
   boundaries), then `npm run dev:all` and confirm the new screen mounts with zero
   console errors.
