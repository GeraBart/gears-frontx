# Workflow: Add an MFE Package (frontx-template-standard)
...
each step names the concrete command or file this template ships.

## Preconditions

- Working inside a project scaffolded from `frontx-template-standard`.
...
4. **Rewrite `mfe.json` GTS IDs**
   - Replace the manifest ID, every entry ID, and every extension ID using this
     template's ID taxonomy (`gts-id-conventions` guideline; worked examples in
